...
export default defineConfig({
  plugins: [
    ...
    // auto-imports — see CLAUDE.md). KEEP IN SYNC with nuxt.config.ts's
    // `imports.dirs` + the modules' presets, same convention as
    // MagicTheGathering/league. Nuxt *runtime* composables (useToast,
    // useSupabaseClient, useAsyncData, useRoute, ...) are NOT covered — they
    // need a running Nuxt app; tests must stub/mock those per test.
    AutoImport({
      imports: [
        'vue',
        'vue-i18n',
  ...
  test: {
    ...
    coverage: {
      ...
      exclude: ['app/types/**', 'shared/utils/types/database.ts']
    }
  },
  resolve: {
    alias: {
