...
- **`assign_role(p_user_id uuid, p_role app_role)` created 2026-08-17.** Restricted to `is_super_admin(auth.uid())`, not `is_admin` — matches `manage-roles` being scoped `super_admin`-only (§1's `PERMISSION_LEVEL`). Upserts into `user_roles` (`ON CONFLICT (user_id) DO UPDATE`, since the table is now `UNIQUE(user_id)`); assigning `p_role = 'player'` deletes the row instead of inserting one, matching the existing convention that `player` is just `get_user_role`'s `COALESCE` default and never gets a stored row. `MembersList.vue`'s role dropdown still isn't wired to it (see the TODO note below) — the RPC exists, the UI call doesn't yet. **Audit trail added 2026-08-18** (`supabase/migrations/20260818100000_finish_audit_trail_pattern.sql`): `user_roles` gained `created_by`/`updated_by` (FK to `pauperwave_associates(uuid)`, matching `docs/supabase/2-database.md`'s audit trail pattern), populated inside `assign_role()` itself by resolving the caller's own associate uuid via `players.user_id = auth.uid()` — `created_by` set once on insert, `updated_by` refreshed on every reassignment. Verified via a rolled-back transaction with `request.jwt.claims` set to simulate a real caller.
...
const PERMISSION_LEVEL = {
  ...
  'cancel-round': 'admin', // ordinary round management stays 'organizer', via 'manage-tournaments'; also covers league's "turn back to registration" case
  'delete-commander-deck': 'admin', // deleting someone else's deck — distinct from 'manage-all-commander-decks' (edit/manage, admin)
  'delete-ruleset': 'admin',
  // 'admin' as of 2026-08-23 (2nd pass, user request), not 'super_admin' —
  // this constant only gates whether the page/control is reachable at all.
  // The finer-grained rule ("admin can grant player/organizer/admin, never
  ...
  // itself (migration 20260823130000), which a client-side role-level check
  // can't express.
  'manage-roles': 'admin'
...
**`super_admin`'s reason to exist beyond role assignment, confirmed 2026-08-10, narrowed 2026-08-23: *permanent* deletion, not destructive actions in general.** `admin` can create/edit/manage tournaments, leagues, events, associates; actually *deleting* one of those permanently is reserved one level higher, as a guard against an accidental destructive click rather than against a malicious admin — see `docs/architecture/permissions.md`'s note on this. Explicitly **does not extend to associates**: an associate is never hard-deleted today, membership status is derived from `pauperwave_associate_renewals` (`docs/architecture/database.md`), so there is no "delete an associate" action for this rule to gate in the first place. **Revised 2026-08-23** (user request: `admin` should have every power except permanent deletion): cancelling an already-started round, deleting someone else's Commander deck, and deleting a ruleset were moved down from `super_admin` to `admin` — they're destructive but reversible-in-spirit (a round can be redone, a deck/ruleset recreated), unlike a hard `delete()` on `/trash` or a tournament/league/event.

**`manage-roles`, revised again the same day (2nd pass, user request): `admin` too, not `super_admin`-only — but with the `super_admin` tier itself carved out, at the RPC, not the permission constant.** The rejected middle ground was "block only self-promotion" — trivially bypassable (an `admin` promotes a second account to `admin`, then uses *that* account to promote the first to `super_admin`), so the real boundary has to be "an `admin` can never touch the `super_admin` tier at all," not "can't touch their own role." `assign_role` (migration `20260823130000`, revised again the same day in `20260823140000` — see below) now: (1) allows any `admin`-or-above caller, not just `super_admin`; (2) rejects the call outright if the target `p_role` is `'super_admin'` **or** the target user's *current* role already is `'super_admin'`, unless the caller is `super_admin` themselves; (3) unconditionally rejects any change to a row with `user_roles.role_locked = true`, regardless of caller role or requested role.

**`role_locked` (`user_roles`, migration `20260823140000`), not a hardcoded uuid — 3rd pass, user request the same day.** `20260823130000` shipped the "protect one specific account" rule as a uuid literal baked into `assign_role`'s function body; asked directly whether that was a good idea, and it isn't: fragile (silently stops protecting anyone if that `auth.users` row is ever deleted/recreated), opaque (who's protected is only readable by opening the migration's source, not by querying anything), and rigid (protecting a second account later means another migration, not an `UPDATE`). Replaced with a plain `boolean NOT NULL DEFAULT false` column on `user_roles` — `assign_role` checks `role_locked` instead of comparing `p_user_id` to a literal. The same migration flips it to `true`, once, as a one-time data statement (not function logic) for the app's developer/owner (Emanuele Nardi, `emanuelenardi.magic@gmail.com`, uuid resolved via `supabase.auth.admin.listUsers()`, not guessed) — same "hardcoded constant → DB-backed data" move already applied elsewhere in this app (ADR-020, the membership fee). Nobody, not even another `super_admin`, can flip it back through `assign_role` (no code path touches the column); only a direct write against `user_roles` can, and that table's RLS (`admin_full_access`) stays `is_super_admin`-gated. The `user_roles` RLS policy is intentionally left stricter than the RPC in general: `assign_role` is `SECURITY DEFINER` and is the only sanctioned write path for *role* changes, nothing legitimate needs direct table access at the `admin` tier.
...
12. **Partially done 2026-08-17.** Wired `can()` into `useMainNavGroups.ts` (nav-visibility permission map decided together with the user, see `docs/architecture/permissions.md`'s "Navigazione" section: `view-associates`/`view-finance`/`view-players`/`manage-locations`/`manage-rulesets` at `organizer`, `access-settings` at `admin` — revised down from `organizer` for `/settings`, see below). Groups whose every item gets filtered out are dropped entirely, not left as a dangling header. Verified live in the browser as `super_admin` — full sidebar renders correctly. **Still outstanding:** the wanted-cards "Elimina" button `v-if` and `MembersList.vue`'s promotion dropdown wiring (step 3's RPC exists, nothing calls it from the UI yet).
    - **Real bug found and fixed, reported by the user directly using the app:** an organizer could still reach `/settings/members` and `/settings/domains` by clicking `settings.vue`'s own internal tab bar — a second, separate nav surface from `useMainNavGroups.ts`, completely unfiltered. Root cause was worse than a missing filter on that one bar: **no page under `/settings` declared `definePageMeta({ permission })` at all**, so `authorization.global.ts` never enforced anything there — hiding the sidebar link was UI-only with no route-level backing, exactly the "not a real security boundary" caveat this doc keeps repeating, just not yet applied to this section. Fixed by (1) collapsing `/settings`'s permission model to a single `access-settings: admin` for all five pages (general/members/permissions/domains/notifications, revised down from the finer `organizer`/`admin` split — simpler and the user's explicit call), (2) filtering `settings.vue`'s own tab bar on the same `can('access-settings')`, and (3) adding `definePageMeta({ permission: 'access-settings' })` to every page under `settings/*.vue`, including `notifications.vue` which had no link anywhere but was still reachable by direct URL. Verified via `router.push()` in the browser console (a real client-side SPA navigation, not a hard reload — which would have reset the "view as" preview and defeated the test): previewing as `organizer` and navigating to `/settings/members`, `/settings/domains`, and `/settings` all correctly redirected to `/403`; previewing as `admin` reached all three normally.
...
**Step 12's last outstanding item, done 2026-08-25:** `MembersList.vue`'s role dropdown now calls `assign_role` directly on change (`useMembersMutations.ts`, same "SECURITY DEFINER RPC called straight from the client" precedent as `useUserRole.ts`'s own `get_user_role` call — the RPC already self-guards permission/`role_locked`/the super_admin carve-out internally). The mock `/api/members` roster is gone too: `server/api/settings/members.get.ts` now reads real account-linked players (`players.user_id is not null`) joined with `user_roles`, gated `requireAdminPermission` — a deliberate BFF read exception, since `user_roles`' RLS only lets a caller read their own row or, once already super_admin, every row, so an admin managing roles couldn't otherwise see anyone else's. The vestigial "Modifica"/"Rimuovi" dropdown items (template scaffold leftovers, never wired to anything real in this domain) were dropped rather than wired up.
