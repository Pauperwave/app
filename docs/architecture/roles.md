...
- `ensureRole()` — idempotent: returns immediately if `status === 'ready'`, returns the in-flight `Promise` if already `'loading'`, otherwise calls `fetchRole()`. Idempotency needs the `Promise` itself kept somewhere both the plugin and the middleware can see, not just the status — kept as a plain module-level `let _pendingFetch: Promise<void> | null`, a deliberate choice between two imperfect options, not an oversight:
  - A module-level `let` is a Node-process-wide variable, shared across every concurrent SSR request — not per-request like `useState`. For this app's traffic (an association's internal tool, not a multi-tenant SaaS) the overlap window is milliseconds and the consequence is low; accepted as-is.
  ...
  - The actual fix for both problems at once — request-scoped **and** not serialized — is attaching the `Promise` to `useNuxtApp()` (e.g. a field Nuxt doesn't serialize, unlike `useState`) instead of either a module `let` or `useState`. Not worth it here: documented module-level trade-off over an extra layer of indirection, for an internal tool's traffic.
