...
| Alert admin (nuovo socio, pagamento, ecc.) | — (solo staff, ruolo `admin`/`super_admin`) | 🟡 | `notifyTelegramAdmins()` esiste ed è pronta all'uso, ma nessun evento dell'app la chiama ancora — nessun alert reale viene inviato oggi. **Decisione 2026-09-02: niente `TELEGRAM_ADMIN_CHAT_ID` da env var** — i destinatari vanno risolti interamente dal database (`pauperwave_associate_telegram_links` join sul ruolo, stessa RPC di `requireAdminPermission`), non da una variabile d'ambiente statica. |
