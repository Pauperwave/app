...
| Alert admin (nuovo socio, pagamento, ecc.) | — (solo staff, ruolo `admin`/`super_admin`) | 🟡 | `notifyTelegramAdmins()` esiste ed è pronta all'uso, ma nessun evento dell'app la chiama ancora — nessun alert reale viene inviato oggi. **Decisione 2026-09-02: niente `TELEGRAM_ADMIN_CHAT_ID` da env var** — i destinatari sono risolti da `get_admin_telegram_chat_ids()` (funzione Postgres, migrazione `20260902102812`), che fa il join `user_roles → players → pauperwave_associate_telegram_links` in una sola query — non tre round-trip separati (`user_roles.user_id` e `players.user_id` non hanno una FK diretta tra loro, PostgREST non può annidare il select senza passare da una funzione). |
