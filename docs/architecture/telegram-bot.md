...
| Collegamento chat↔socio | Collegato | 🔴 | Al primo `/start`, il bot chiede l'email associata (skippabile con `/skip` — resta usabile solo con i comandi pubblici). Verifica su `pauperwave_associates`, salva `chat_id ↔ associate_uuid` in una nuova tabella `pauperwave_associate_telegram_links`. Rate-limit sui tentativi falliti per `chat_id` da aggiungere (email da sola è indovinabile/nota). Non ancora un comando a sé — resta dentro `/start`, non stubbato separatamente. |
