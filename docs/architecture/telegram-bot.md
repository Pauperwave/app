...
| Collegamento chat↔socio | Collegato | 🔴 | Al primo `/start`, il bot chiede l'email associata (skippabile con `/skip` — resta usabile solo con i comandi pubblici). Verifica su `pauperwave_associates`, salva `chat_id ↔ associate_uuid` in una nuova tabella `pauperwave_telegram_links`. Rate-limit sui tentativi falliti per `chat_id` da aggiungere (email da sola è indovinabile/nota). Non ancora un comando a sé — resta dentro `/start`, non stubbato separatamente. |
...
| Alert admin (nuovo socio, pagamento, ecc.) | — (solo staff, `TELEGRAM_ADMIN_CHAT_ID`) | 🟡 | `notifyTelegramAdmins()` esiste ed è pronta all'uso, ma nessun evento dell'app la chiama ancora — nessun alert reale viene inviato oggi. |
| Tavolo/avversario per turno | Collegato | ⚫ | **Non costruibile oggi**: l'app non ha alcun concetto di pairing/turno (`app/types/index.d.ts` — `Tournament` ha solo `roundCount`, le iscrizioni hanno solo stato `registered`/`checked_in`). Richiederebbe un intero sistema di abbinamenti (motore Swiss, inserimento risultati per turno) prima ancora di poter essere un comando bot. Comando `/tavolo` registrato, risponde spiegando il blocco (`commands/stubs.ts`), non un generico "da implementare". |
