...
**"Carte Cercate": corretto 2026-09-05 (era 🔴 dal 2026-08-10, vedi cronologia sotto).** `server/api/wanted-cards/create.post.ts` usa solo `requireUser` (chiunque loggato crea la propria richiesta). `[id]/status.post.ts` e `[id]/delete.post.ts` ora usano `requireManagementOrWantedCardOwner` (`server/utils/wantedCards.ts`): il creatore di una richiesta (owner-check su `player_associate_uuid`, risolto via `resolveAuditAssociateUuid`) può segnarla "trovata"/"abbandonata" **ed eliminarla**, senza bisogno di un ruolo di gestione; chiunque altro resta gated su `has_management_permissions`, invariato. Stesso comando lato bot Telegram (`/cartecercate`, `commands/cartecercate.ts`) — un chat collegato può gestire solo le proprie richieste. **Cronologia:** la decisione 2026-08-10 aveva corretto solo `status.post.ts`, lasciando `delete.post.ts` riservato alla gestione ("l'unica cosa che deve restare riservata alla gestione è l'eliminazione"); richiesta utente 2026-09-05 ha esteso l'auto-gestione anche all'eliminazione (soft delete, recuperabile da `/trash` come ogni altra). `[id]/update.post.ts` e `[id]/refresh-prices.post.ts` restano `requireManagementPermission`-only, fuori scope da entrambe le decisioni.
...
**Mazzi Commander, due righe separate:** gestire i **propri** mazzi è 🟢 per chiunque, sempre (`player_own_decks` ownership-check, indipendente dal ruolo); gestire i mazzi **di chiunque altro** è una capacità a parte, riservata ad admin e super-admin.
...
Ogni volta che si aggiunge un `Permission` a `PERMISSION_LEVEL` in `app/utils/permissions.ts` (`docs/architecture/roles.md`), aggiungere una riga qui — sono la stessa fonte di verità raccontata in due formati diversi (codice vs. tabella leggibile), non due decisioni separate.
