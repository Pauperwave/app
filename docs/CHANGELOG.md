...
- The "Premi" (awards) step is now Commander-only; in the 1v1 table the drop is a row action in an "Azioni" column.
- Round count follows the round duration's shape (`feat(settings): ✨ set the round count per format family`, migration `20260920030000`): 4 rounds for every 1v1 format, 2 for Commander, both in `/settings`; the per-format map is gone (ADR-035).
- 1v1 round view polish (`feat(tournaments): ✨ cancel a match result and flag the tables still to enter`): an "Annulla inserimento" button (red, outlined) removes a table's result and puts the pairing back to pending, with an optimistic update that cancels in-flight refetches first; tables and rows waiting for a result get a yellow ring/tint and a "Da inserire" badge in the same header slot the button takes (no layout shift), completed ones fade back; standings rows of players still waiting are tinted yellow with a legend in the card header, show "–" instead of the 33.3% tiebreak floor before a first match, and can be searched (real rank kept); the drop moved from a button next to the name to a per-player menu plus a read-only badge and a struck-through name.
- `feat(tournaments): ✨ explain why the advance button is disabled`: a tooltip on the disabled "Prossimo round"/"Termina torneo" button (shared by both round managers).
...
## 2026-09-19 – 2026-09-20 — Prize distribution reworked; the stepper stops opening unreached steps; the version badge shows the app's age

- The prizes step became a real tool (`feat(prizes): ✨ rework the prize distribution step`): two columns (settings + chart | presets + standings), a percentage and a pack count per rewarded placement that stay linked (a share step moves exactly one pack, the shares are derived from the packs actually assigned, every pack stays assigned), the non-rewarded players in a collapsible section (closed by default), and a green/red tint on the rows whose packs just changed. Not persisted on purpose; see ADR-039 and `architecture/prize-distribution.md`.
- New settings, drawn as dashed reference lines in the chart: a minimum for the non-rewarded players, packs set aside, and a cap per placement (7 by default). Every control disables its +/− at its bound with a tooltip saying why, has an "i" tooltip explaining what it does and its own reset button; "Ripristina" resets them all, "Personalizzato" restores the organizer's last hand-edited shares (`useStorage`, localStorage).
- The order is now a hard rule: a lower placement never has more packs than a higher one (`fix(prizes): 🐛 never give a lower placement more packs than a higher one`), and the non-rewarded minimum can't exceed the rewarded one (`fix(prizes): 🐛 never let a non-rewarded player get more packs than a rewarded one`). Packs move whole, by rank rather than by rounding (`refactor(prizes): ♻️ decide who gives or takes a pack by rank`).
- Refactors along the way: the logic split into pure, tested utils (`prizeBudget`/`prizeAllocation`/`prizeShares`/`prizeLimits`), the step's state in `usePrizeDistributionPage`, and generic `PresetButtons` (also used by the pairing presets), `HintedNumberField`, `ValueStepper` and `useChangeFlash` extracted into `ui/`/`composables/`. `fix(prizes): 🐛 show the tooltips on the disabled settings controls`: `UInputNumber` drops `UTooltip`'s trigger listeners, so it is wrapped in a native `div`.
- `fix(tournaments): 🐛 don't let the stepper open steps the tournament hasn't reached`: every step stays visible but the ones ahead are disabled in production (and a `?step=` link to one is ignored), except in development and with the developer view on (`feat(tournaments): ✨ let the developer view open any stepper step in production`). See ADR-040.
- `feat(layout): ✨ show how long ago the app was updated in the version badge`: the age of the deployed commit as one unit (minutes, hours, days, months, years) with the exact date on hover, via the tested `timeSince` util.
- Smaller: `feat(tournaments): ✨ add a cancel button and icons to the table-preview footers` (the Swiss preview gets the same Cancel + Confirm pair as the Commander one).
