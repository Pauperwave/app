...
/** Query key for a batch commander-card lookup by name set. */
export const COMMANDERS_BY_NAMES_KEY = ['commanders-by-names']

/**
 * Batch commander-card lookup for a set of names, gated by `enabled`. Colada
 * caches by the sorted name set, so re-selecting the same set never refetches.
 */
...
/** A deck's commander card(s) from `mtg_commanders`, keyed reactively by name. */
