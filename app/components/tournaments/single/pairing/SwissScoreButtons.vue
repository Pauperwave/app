...
function matches(outcome: { won: number, lost: number }): boolean {
  if (!current) return false
  const target = scoreFor(outcome)
  return current.player1GamesWon === target.player1GamesWon
    && current.player2GamesWon === target.player2GamesWon
}
</script>
