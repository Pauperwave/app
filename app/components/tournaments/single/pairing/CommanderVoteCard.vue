...
const {
  ...
  playerUuid?: string
  selected?: boolean
  /** Roving tabindex — the parent keeps exactly one card at 0, the rest at -1. */
  tabindex?: number
}>()
...
// Self-contained: reads the already-cached (30-day) commander catalog — the
// same one CommanderSearch autocompletes against — instead of requiring the
// caller to resolve art/mana cost itself.
...
<template>
  <div
    ...
    :aria-label="commanderName
      ...
      : `${t('tournament.single.commanderModal.voteCard.assignCommander')} — ${name} ${surname}`"
    @click="onActivate"
    @keydown="onKeydown"
  >
    <UCard class="overflow-hidden" :ui="{ body: 'p-0 sm:p-0', footer: 'px-2.5 py-2' }">
      <div class="aspect-4/3 bg-muted">
        ...
        <div v-else class="h-full w-full flex flex-col items-center justify-center gap-1.5 p-2 text-warning">
          <UAvatar
            size="lg"
            :src="avatarUrl || generatePlayerAvatar(playerUuid ?? `${name} ${surname}`)"
            :alt="name"
          />
          <span class="flex items-center gap-1 text-xs font-medium">
            <UIcon :name="ICONS.commander" class="size-4" />
            {{ t('tournament.single.commanderModal.voteCard.assignCommander') }}
          </span>
        </div>
      </div>
      <template #footer>
