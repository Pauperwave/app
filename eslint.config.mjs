...
export default withNuxt(
  ...
  },
  {
    ...
    rules: {
      ...
      // Style (comma-dangle/brace-style come from nuxt.config.ts's
      // eslint.config.stylistic option, applied to the base @nuxt/eslint config)
      ...
      // 100 chars — ignoreUrls/Strings/TemplateLiterals: a long i18n key
      // shouldn't force an awkward wrap just to hit a number. ignoreComments:
      // JSDoc/comment lines aren't code and don't benefit from mid-sentence
      // wraps. ignorePattern for `class="..."`: Tailwind strings are left
      // as-is rather than broken — line-wrapping a class list doesn't
      // improve readability the way wrapping code does.
      ...
      // 2-space indent, enforced. @stylistic/indent covers <script> content in
      // both .ts and .vue files; vue/html-indent covers the <template> block,
      // which @stylistic/indent doesn't parse. SwitchCase: 1 matches this
  ...
  {
    // shared/utils/types/database.ts is generated (`pnpm supabase:types`),
