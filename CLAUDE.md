...
### UTable's imperative `column.setFilterValue()` races against its own `v-model:column-filters`
A `UTable` bound with `v-model:column-filters="columnFilters"` treats that ref as the declarative source of truth for its filter state — but nothing stops a page from *also* fetching a column off `table.value?.tableApi?.getColumn(id)` and calling `.setFilterValue()` on it imperatively. Doing both on the same table works for a single filter change, but if one function call makes **two** real filter changes across two different columns in the same synchronous tick, the second one loses the race against `UTable`'s own prop-watcher re-syncing the (still-stale) `columnFilters` ref back into the table — silently reverting it.

Confirmed 2026-09-14 in `associates/index.vue`'s `applyMembershipStatusFilterFromQuery`: switching `?status=pending_renewal` → `?status=active` calls `pendingRenewalColumn.setFilterValue(undefined)` then `statusColumn.setFilterValue('active')` — two columns, two real changes (unlike the *first* visit to `pending_renewal`, where `statusColumn.setFilterValue(undefined)` was a no-op since it was already `undefined`, so only one real change happened and nothing raced). The table kept showing only the pending-renewal row no matter what, regardless of the search box — reproduced with an unrelated search term to confirm the stuck filter wasn't the global search at all, just the `has_pending_renewal` column filter.

Fix: don't imperatively call `column.setFilterValue()` on a table also controlled by `v-model:column-filters` — replace `columnFilters.value` wholesale instead (one source of truth, no dual-write to race). `associates/requests.vue`'s own status filter and its status-badge click handler (`useAssociatesTableColumns.ts`) were checked and are safe: each only ever touches one column per call, so there's never a second write in the same tick to lose a race against.
...
### @grammyjs/menu can only be sent via `ctx`, not `bot.api`
A `Menu` instance's `reply_markup` can only be rendered by sending it through a context object (`ctx.reply`, `ctx.api.sendMessage`, ...) — never through the bare `bot.api` client, even though `bot.use(theMenu)` is already registered. The plugin's own source says as much (`@grammyjs/menu/out/menu.js`'s `inline_keyboard` getter): *"Sending the menu is not directly possible via bot.api.sendMessage, only via the context object, at least not yet."* Calling it the wrong way doesn't warn at the call site — it throws at send time: `Cannot send menu 'xyz'! Did you forget to use bot.use() for it or try to send it through bot.api?`.

Confirmed 2026-09-09 in `server/utils/telegram/commands/mockups/tavolo.ts`: `pushTavoloMessage` was refactored to take a bare `chatId` (so it could later be called from a ctx-less server trigger) and sent via `useTelegramBot().api.sendMessage(...)` — this broke `/tavolo` in production with exactly that error, caught via `vercel logs` after a user report. Fix: take `ctx` and use `ctx.api.sendMessage(...)` instead of `bot.api.sendMessage(...)`. Practical implication: a menu-bearing message genuinely cannot be pushed from a true ctx-less trigger (e.g. a cron job or DB webhook with no in-flight Telegram update) until the plugin adds that support — only reachable today from within an existing update handler, even to send to a *different* `chatId` than the one that triggered it.

### @grammyjs/menu: registration order determines which menus a `next()`-skipping handler can send
Each `Menu`'s "permission to send this menu" is installed via `ctx.api.config.use(...)` *inside its own middleware*, once per update (confirmed 2026-09-09 in `@grammyjs/menu/out/menu.js:570`). If a `bot.on(...)` handler registered *before* a later `register*Command`'s own `bot.use(<thatMenu>)` fully handles an update without calling `next()`, that later menu's middleware never runs for this update — its permission never gets installed — and any attempt to send a message with that menu as `reply_markup` from within the earlier handler throws `Cannot send menu 'x'! ... try to send it through bot.api?`, even though the exact same code works fine when reached through its own command.

Confirmed 2026-09-09: `/help`'s quick-launch buttons reused `/calendario` and `/prossimo`'s own deep-link handlers (`resolveDeepLink(...)`, see `deepLinks.ts`), which reply using `calendarioMenu`/`prossimoMenu`. The button handler (`handleHelpButton`, registered inside `registerCoreCommands`, which ran *first* in `commands/index.ts`'s `registerCommands`) never called `next()`, so `registerCalendarioCommand`/`registerProssimoCommand`'s own `bot.use(...)` — registered later — never ran for that update. Typing `/calendario` directly worked fine (nothing shadowed its own `bot.use(calendarioMenu)`); the button crashed.

Fix: any `bot.on(...)` handler that doesn't call `next()` and might reuse another command's menu-bearing reply must be registered *after* every `register*Command` whose menu it could need — see `registerHelpButtonHandler`, split out of `registerCoreCommands` and called last in `registerCommands`, once every other menu has had a chance to run for the update at least once.
...
Another distinct auto-import collision confirmed 2026-09-14 in `app/components/tournaments/single/AcceptancePicker.vue`: calling VueUse's `useStorage()` (from `@vueuse/nuxt`) relying on auto-import resolved to Nitro's own server-side `useStorage()` (unstorage's `Storage<T>`) instead — `pnpm typecheck` failed with `Storage<T>` type errors (wrong overload, wrong arg count), and skipping typecheck surfaces as a runtime `ReferenceError: useStorage is not defined` in the browser. Both composables share the name in the shared auto-import namespace, and the wrong one wins. Fix: add an explicit `import { useStorage } from '@vueuse/core'` rather than relying on auto-import — same workaround pattern as the `ICONS` case above, different root cause (name collision between two auto-import sources, not the array-literal-export bug).
...
### Multi-item actions must show the count in the label
Whenever a button or menu item's action applies to more than one item (a `BulkActionsBar.vue`'s own action buttons/dropdown triggers, or any other multi-select "do X to the selection" control), the label itself must include how many items it will act on — not just an adjacent "N selezionati" text, which a dropdown/context-menu item has no room for at all. Use the shared `withCount(label, count)` util (`app/utils/withCount.ts`, `${label} (${count})`) rather than hand-formatting the string inline. Established 2026-09-23 (user request), applied to every domain's `list/BulkActionsBar.vue` (players, associates, transactions, tournaments, leagues, events, wanted-cards) and `AcceptancePickerToolbarRow.vue`'s "Rimuovi selezionati".

Not applied to a dropdown's own leaf *value-selection* items (e.g. the individual status options inside "Segna come (N)") — the trigger already carries the count, and the confirm step that follows (when there is one) shows it again in its own title. Also not applied to a "Seleziona tutti" button — it doesn't act on the current selection, it replaces it.
